...
describe('PageService', () => {
  ...
  it(
    'should get list of pages for path',
    waitForAsync(() => {
      pageService.getRootDirectoryForPath('publisherManagementWS>qa>_constraints_').subscribe(directory => {
      ...
      const req = httpMock.expectOne(
        mockRequest =>
          ...
          mockRequest.url === 'api/directories' &&
          mockRequest.params.get('path') === 'publisherManagementWS>qa>/constraints/'
      );
  ...
  xit(
    'should search pages for keyword',
    waitForAsync(() => {
      pageService.searchPages('suggestions').subscribe(result => {
  ...
  it(
    'should get page for path',
    waitForAsync(() => {
      pageService.getPage('publisherManagementWS>qa>_constraints_overview').subscribe(page => {
      ...
      const req = httpMock.expectOne(
        mockRequest =>
          mockRequest.method === 'GET' &&
          mockRequest.url === 'api/pages' &&
          mockRequest.params.get('path') === 'publisherManagementWS>qa>/constraints/overview'
      );
  ...
  it(
    'should parse markdown to find external link',
    waitForAsync(() => {
      pageService.getPage('publisherManagementWS>qa>_constraints_overview').subscribe(page => {
        expect(page).toBeDefined();
        expect(page.path).toEqual('publisherManagementWS>qa>/constraints/overview');
        ...
        expect((page.parts[0].data as IncludeExternalPagePart).includeExternalPage).toBe(
          'http://publisher.corp.kelkoo.net/docs/#/Contact%20Management/getContact'
        );
      });
  ...
  it(
    'should parse page containing scenarios',
    waitForAsync(() => {
      pageService.getPage('path').subscribe(page => {
        expect(page).toBeDefined();
      ...
      const req = httpMock.expectOne(
        mockRequest => mockRequest.method === 'GET' && mockRequest.url === 'api/pages' && mockRequest.params.get('path') === 'path'
      );
