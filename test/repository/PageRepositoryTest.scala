...
class PageRepositoryTest extends PlaySpec with GuiceOneServerPerSuite with Injecting with BeforeAndAfterEach {
  ...
  val page1 = Page(1, "app", "page1", "RAS", 0, "appMarkdown", "", "project_id1>branch1>", 1)
  val page2 = Page(2, "front", "page2", "aucune description", 1, "frontMarkdown", "", "project_id1>branch1>", 1)
  ...
  override def beforeEach() {
    db.withConnection { implicit connection =>
      pages.foreach { page =>
        SQL"""INSERT INTO page (pageId, name, label, description, `order`,markdown
  ...
  override def afterEach() {
    db.withConnection { implicit connection =>
      ...
      SQL"ALTER TABLE page ALTER COLUMN pageId RESTART WITH 1".executeUpdate()
    }
  }
  ...
  "PageRepository" should {
    ...
    "delete a page by id" in {
      pageRepository.deleteById(page1.pageId)
      db.withConnection { implicit connection =>
        SQL"SELECT COUNT(*) FROM page WHERE pageId = ${page1.pageId}".as(scalar[Long].single) mustBe 0
      }
    }

    "find all by id" in {
      pageRepository.findAllById(pages.tail.map(_.pageId)) must contain theSameElementsAs pages.tail
    }

    "find a page by id" in {
      pageRepository.findById(page1.pageId) mustBe Some(page1)
    }
    ...
    "get all pages by projectId" in {
      pageRepository.findAllByBranchId(1) must contain theSameElementsAs Seq(page1, page2)
    }

    "check if a page exist by id" in {
      pageRepository.existsById(page1.pageId) mustBe true
    }

    "save a page" in {
      val newPage = Page(-1, "assets", "page1", "RAS", 3, "assetsMarkdown", "", "project_id1>branch1>", 1)
      pageRepository.save(newPage) mustBe newPage.copy(pageId = 4)
    }
    ...
    "save all pages by projectId" in {
      ...
      val expectedPages = Seq(page4.copy(pageId = 4), page5.copy(pageId = 5))
