...
class OpenApiClientTest extends WordSpec with MustMatchers with MockitoSugar with ScalaFutures {
  ...
  val expectedSpecWithRefStartsWith =
    """
      {
         "basePath":"/",
         "paths":{
  ...
  "OpenApiClient" should {
    ...
    "parse swagger.json file with path module with refStartsWith" in {
      ...
      Server.withRouterFromComponents()(cs => {
        ...
        WsTestClient.withClient { client =>
          val openApiClient = new OpenApiClient(client)
          val result = Await.result(openApiClient.getOpenApiPathSpec(pathModuleWithRefStartsWith, pageJoinProject), 10.seconds)
          result.mustBe(expectedPathWithRefStartsWith)
        }
      }
    }
